...
function isAlmostSame(num1, num2) {
 ...
 else{return false}// Визначаємо, чи є різниця між двома числами меншою або рівною EPSILON
}
...
function isSafeInteger(num) {
 if(num<Number.MAX_SAFE_INTEGER&&num>=Number.MIN_SAFE_INTEGER){return true} // Перевіряємо, чи є число меншим або рівним MAX_SAFE_INTEGER і більшим або рівним MIN_SAFE_INTEGER
 else{ return false}
}
...
function isUnsafeInteger(num) {
  if(!(num>Number.MAX_SAFE_INTEGER||num<=Number.MIN_SAFE_INTEGER)){return true} // Використовуємо логічну оператор НЕ (!), щоб отримати протилежну відповідь від функції isSafeInteger()
 else{return false}
}
...
function isTooLarge(num) {
  if(num>Number.MAX_VALUE){return true}// Перевіряємо, чи є число більшим за MAX_VALUE
 else{return false}
}
...
function isAlmostZero(num) {
 if(num>0&&num<Number.MIN_VALUE){return true} // Перевіряємо, чи є число більше 0, але все ще менше за Number.MIN_VALUE
 else{return false}
}
